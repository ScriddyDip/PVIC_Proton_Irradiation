...
saveFig = True
path = r"C:\Users\Zach\Documents\RJE\DDD_analysis\Plots\DDDCalc_2\DDDCalc_2_0"
...
# Excitation_Incidence = "filmside"

Solar_cell_parameter = "Median(Efficiency (%))"
Scp_uncertainty = "Std Dev(Efficiency (%))"

df = pd.read_csv(r"C:\Users\Zach\Documents\Python Scripts\DDD\Task 24_AuburnSamples_JVSummary_SampleLevel_forDDD.csv")
NIEL_df = pd.read_csv(r"C:\Users\Zach\Documents\Python Scripts\DDD\dataNIEL.csv")
...
def DamageCurveGrabber(df, energy, dopant, excitation_incidence, solar_cell_parameter, scp_uncertainty):
    buffer_df = df.copy()
    buffer_df = buffer_df.loc[(df["Excitation Incidence"] == excitation_incidence) & (df["Proton Acceleration Voltage (kV)"] == energy) & (df["CdTe p-Type Dopant"] == dopant)]
    ...
    for ID in sampleIDList:
        ...
        try:
            ...
            fluence = temp_df["Fluence (# cm^-2)"].unique()
            fluence = eval(fluence[0])
            uncertainty_fluence = (0.1)*fluence
            data_point = ([fluence],[remaining_factor],[uncertainty_Reff],[uncertainty_fluence],[ID])
...
for Energy in energylist: 
    ...
    col = next(color)
    ax.errorbar(x=data[:,0][:,0],y=data[:,1][:,0],yerr=data[:,2][:,0],xerr=data[:,3][:,0], marker = 's',color = col,ecolor="black",capsize=3, label=f"{eval(Energy)*(1e-3)} MeV",linestyle='')
...
ax.set_xscale('log')
if saveFig:
        plt.savefig(path+f"\{Dopant}_{Excitation_Incidence}_{energylist[0]}to{energylist[-1]}_"+Solar_cell_parameter+"_fluence.png",dpi = 400)
...
curve_fit_abscissa = np.logspace(np.log10(DDD_all_energy_DDD_data[0]),np.log10(DDD_all_energy_DDD_data[-1]),100)
...
if saveFig:
        plt.savefig(path+f"\{Dopant}_{Excitation_Incidence}_{energylist[0]}to{energylist[-1]}_"+Solar_cell_parameter+"_DDD.png",dpi = 400)
