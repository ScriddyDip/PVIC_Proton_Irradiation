...
for Dopant in DopantList:
    ...
    for energy in energyList:
        ...
        individualdataList.sort(key=lambda x: x[0])
        individualdataList = np.array(individualdataList)
        print(individualdataList)
        ...
        popt, pcov = curve_fit(fitting_function, individualdataList[:,0], individualdataList[:,1])
    ...
    for energy in energyList:
        ...
        buffer_df = damage_Vs_fluence_curve_dict_individualdata[energy]
    ...
    if no100Toggle:
        ...
        for energy in energyList[1:]:
            ...
            c = 'black'
            MeV_energy = (1/1000)*energy
            buffer_no100 = damage_Vs_fluence_curve_dict_individualdata[energy]
            
            for dataPoint in buffer_no100:
                no100List.append(dataPoint)

            ax.scatter(buffer_no100[:,0],buffer_no100[:,1], marker = "o", label = "_label_", color = c)
...
for Dopant in DopantList:
    ...
    if plotTitlesEqsBool:    
        fig.suptitle(r"$\bf{H^+}$ Irradiated "+f"{Excitation_Incidence} CdSeTe Efficiency Degredation.png", fontsize = 18,weight = 'bold')
        props = dict(boxstyle='round', facecolor='white', alpha=0.5)
        fig.text(0.2,0.45, r"$y=1-C*log(1+\frac{x}{D_x})$", fontsize = 15,bbox=props)
    fig.tight_layout()
