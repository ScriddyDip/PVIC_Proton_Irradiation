...
saveFig = True
titleBool = True
path = r"C:\Users\Zach\Documents\RJE\DDD_analysis\Plots\DDDCalc_2\DDDCalc_2_1"
...
solar_cell_parameter_list = ["Median(Efficiency (%))","Median(Voc (V))","Median(Jsc (mA/cm^2))","Median(Fill Factor (%))","Median(Rs (Ohms *cm^2))","Median(Rshunt (Ohms * cm^2))"]


scp_uncertainty_list = ["Std Dev(Efficiency (%))","Std Dev(Voc (V))","Std Dev(Jsc (mA/cm^2))","Std Dev(Fill Factor (%))","Std Dev(Rs (Ohms *cm^2))","Std Dev(Rshunt (Ohms * cm^2))"]

df = pd.read_csv(r"C:\Users\Zach\Documents\VSCodeFolders\Python Scripts\DDD\Task 24_AuburnSamples_JVSummary_SampleLevel_forDDD.csv")
