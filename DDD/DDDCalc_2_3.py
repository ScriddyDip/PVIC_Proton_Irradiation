...
saveFig = True
titleBool = True
path = r"C:\Users\Zach\Documents\RJE\DDD_analysis\20240708_CdSeTe_Auburn_Sabin\Plots\DDDCalc_2\DDDCalc_2_3"
...
def DamageCurveGrabber(df, energy, dopant, solar_cell_parameter, scp_uncertainty):

    buffer_df = df.copy()
    buffer_df = buffer_df.loc[(df["Proton Acceleration Voltage (kV)"] == energy) & (df["Lightsoak (hours)"] == float(0)) & (df["CdTe p-Type Dopant"] == dopant)]
...
for Dopant in dopant_list:
    for supreme_index,Solar_cell_parameter in enumerate(solar_cell_parameter_list):
        ...
        for Energy in energylist: 
            Energy = eval(Energy)
            data = DamageCurveGrabber(df,energy=Energy,dopant=Dopant,solar_cell_parameter=Solar_cell_parameter,scp_uncertainty=Scp_uncertainty)
        ...
        ax.set_xlabel(r"Fluence [$\mathbf{H}^+$ ions$\mathbf{/cm}^2$]", weight = 'bold', fontsize = 15)
        ax.set_xscale("log")
        ax.set_ylim(0,1.2)
        ...
        ax.set_xscale('log')
        if titleBool:
            fig.suptitle(f"{Dopant} "+re.sub('[^A-Za-z0-9]+', '', Solar_cell_parameter))
        if saveFig:
            plt.savefig(path+f"\sabin_{Dopant}_{energylist[0]}to{energylist[-1]}_"+re.sub('[^A-Za-z0-9]+', '', Solar_cell_parameter)+"_fluence.png",dpi = 400)
            plt.close()
        ...
        popt, pcov = curve_fit(fitting_function, DDD_all_energy_DDD_data, DDD_all_energy_Rf_data, sigma=DDD_all_energy_Rf_uncertainty_data, maxfev=100000)
        ...
        if saveFig:
                plt.savefig(path+f"\sabin_{Dopant}_{energylist[0]}to{energylist[-1]}_"+re.sub('[^A-Za-z0-9]+', '', Solar_cell_parameter)+"_DDD.png",dpi = 400)
                plt.close()
        print(f"\{Dopant}_{energylist[0]}to{energylist[-1]}_"+re.sub('[^A-Za-z0-9]+', '', Solar_cell_parameter))
plt.show()