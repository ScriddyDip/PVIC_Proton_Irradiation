...
def damage_curve_correlator(energy, dopant, uncorrelated_dataset_df, curve_storage_dict=None, excitation_incidence="sunnyside"):
    ...
    correlated_dataset_df = uncorrelated_dataset_df[
        (uncorrelated_dataset_df['Proton Energy (keV)'] == energy)
...
sample_data_df = pd.read_csv(r"C:\Users\slamb\OneDrive\Desktop\Displacement Damage Dose\DDD\DDDCalc_2_5\JV-RF_Sb2S3_forDDD.csv")
absorber_niel_df = pd.read_csv(r"C:\Users\slamb\OneDrive\Desktop\Displacement Damage Dose\DDD\DDDCalc_2_5\sb2s3_proton_niel.csv")
dopant_list = ["Copper"]
markerstyle_dict = {"Copper": "o"}
linestyle_dict = {"Copper": "solid"}

parameter_list = ["PCE-RF (of unity)"]
...
for dopant in dopant_list:    
    ...
    # for loop to loop over energies for fitting degradation curves
        for energy in energy_list:
            ...
            #Define color and shape of markers
            c = next(color)
            axs[n,figure_index].scatter(ddd_list,remaining_factor_list,color=c, marker = markerstyle_dict[dopant], label=f"{energy*(1e-3)} MeV")
    ...
    #fitting of data points and calculation of r-squared
        ...
        print(p)
        print(popt)
        curve_fit_abscissa = np.logspace(np.log10(ddd_cumulative_list[0]),14,100)
        ...
        #assemble plot
        axs[n,figure_index].plot(curve_fit_abscissa,fit_y_values, color = "black", linestyle = linestyle_dict[dopant], label = "\n"+f"C = {popt[0]:.3f}, "+r"$D_x$ = "+f"{popt[1]:.2e} MeV/g: " +r"$R^2=$" +f"{r_squared:.4f}", linewidth=2)
        axs[n,figure_index].set_ylabel(f"{label_p} Remaining Factor", weight = 'bold', fontsize = 15)
        ...
        if n == 1:
            if figure_index == 0:
                ...
                labels = ["CdSeTe:Cu, 0.65 MeV $p^{+}$","CdSeTe:Cu, 1.00 MeV $p^{+}$","CdSeTe:Cu, 1.50 MeV $p^{+}$", "CdSeTe:Cu Fitted Curve","CdSeTe:As, 0.65 MeV $p^{+}$","CdSeTe:As, 1.00 MeV $p^{+}$", "CdSeTe:As, 1.50 MeV $p^{+}$","CdSeTe:As Fitted Curve"]
                axs[n,figure_index].legend(handles[::-1], labels[::-1], fontsize = 11)
                axs[n,figure_index].legend(handles = handles, labels = labels, markerfirst = True)

                
                
                
                '''removed_element = labels.pop(3)
                print(f"Removed element: {removed_element}")
                print(f"Updated list: {labels}")
                labels.append(removed_element)
                print(f"Appended element: {removed_element}")
                print(f"Updated list: {labels}")                
